...
def make_call(target: TargetPodInfo, params: dict):
    ...
    debug_dict = {"target": target_host, "params": params}
...
def create_router(get_config: Callable[[], Awaitable[dict]]) -> APIRouter:
    ...
    async def call(
        ...
        result = make_call(pod_info, params)

        # TODO: do this. we want to redact on message call results, not other call results (but we can EAFP)
